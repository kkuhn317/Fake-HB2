...
public class RollBetter : MonoBehaviour
{
    public float speed = 0;
    public float airSpeed = 0;
    public float slopeSpeedFactor = 2;
	...
	private void FixedUpdate()
	{
        ...
        if (grounded) {
            // find the floor angle using the contact points
            float floorAngle = 0;
            foreach (Vector3 hitPoint in hitPoints) {
                floorAngle += Mathf.Atan2(hitPoint.y, Mathf.Sqrt(hitPoint.x * hitPoint.x + hitPoint.z * hitPoint.z));
            }
            floorAngle /= hitPoints.Count;

            //convert to steepness (1 = flat, 0 = vertical)
            float steepness = Mathf.Abs(floorAngle / (Mathf.PI / 2));
            
            //float testMaxVelocity = maxVelocity / steepness;
            //utilize slopeSpeedFactor
            float testMaxVelocity = maxVelocity / Mathf.Pow(steepness, slopeSpeedFactor);
            
            print(testMaxVelocity);


            if (rb.velocity.magnitude > testMaxVelocity) {
                
                // set velocity to testMaxVelocity
                rb.velocity = rb.velocity.normalized * testMaxVelocity;
            }
